...
quizapp.controller("quizctrl", function($scope, $http, $window, $interval) {
	...
	$scope.SubmitTest = function() {
		...
		for(var i = 0; i < $scope.PastQuestions.length; i++) {
			$scope.PastQuestions[i]['Username'] = $scope.Username;
	...
	$scope.RecordTest = function() {
		...
		if(testready) {
			...
			var TestRecord = {};
			TestRecord['Username'] = $scope.Username;
			TestRecord['TimeTaken'] = $scope.TimeTaken;
			TestRecord['NoteID'] = $scope.ContentID;
			TestRecord['Result'] = $scope.TestResult;
			$http.post("main.php", {RecordTest : true, QuestionRecords : $scope.PastQuestions, TestRecord : TestRecord, AccessToken : $scope.AccessToken, Username : $scope.Username});
	...
	$scope.SubmitAnswer = function() {
		$interval.cancel($scope.TimeInterval);
		$scope.CurQuestion['Username'] = $scope.Username;
		...
		$http.post("main.php", {RecordQuestion : true, QuestionRecord : $scope.CurQuestion, AccessToken : $scope.AccessToken, Username : $scope.Username});
	}
	...
	$scope.GetQuestion = function() {
		$http.post("main.php", {GetQuestion : $scope.ContentType, ContentID : $scope.ContentID, Username : $scope.Username, Override : $scope.Override, AccessToken : $scope.AccessToken}).then(function(response) {
	...
	$scope.GetTestQuestions = function() {
		$http.post("main.php", {GetTestQuestions : true, ContentID : $scope.ContentID, Username : $scope.Username, Override : $scope.Override, AccessToken : $scope.AccessToken}).then(function(response) {
